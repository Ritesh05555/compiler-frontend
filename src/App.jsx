...
// // ... (keep all other imports and components unchanged until EditorScreen)
...
//   const { language } = useParams(); // Use language from path for editor route
...
//   useEffect(() => {
//     if (isShared && token) {
//       console.log('EditorScreen: Fetching shared code for token=', token);
//       setIsLoading(true);
//       axios
//         .get(`https://compiler-backend-e3eg.onrender.com/api/code/${token}`, {
//           timeout: 30000, // Increased timeout to handle slow responses
//         })
//         .then((response) => {
...
//             setCode(sharedCode);
//             setLinkId(token);
//             // No need to set shareLink here as it's set in handleSave
//           } else {
...
//         })
//         .catch((error) => {
//           console.error('EditorScreen: Failed to load shared code:', error.message, error.response?.data);
//           setCode(templates[language] || '');
//           setFetchError('Failed to load shared code. Error: ' + (error.response?.data?.error || error.message));
//           setIsLoading(false);
//         });
//     }
//   }, [isShared, token, language]);
...
//     try {
//       const response = await axios.post('https://compiler-backend-e3eg.onrender.com/api/code/execute', { code, language }, {
...
//     try {
//       const response = await axios.post('https://compiler-backend-e3eg.onrender.com/api/code/save', { code, language }, {
//         headers: { 'Content-Type': 'application/json' },
//         timeout: 15000, // Increased timeout
//       });
//       const { token } = response.data;
//       setLinkId(token);
//       // Update share link to use /api/code/:token path
//       setShareLink(`https://compiler-backend-e3eg.onrender.com/api/code/${token}`);
//       setIsSaved(true);
//       console.log('Save successful, token:', token, 'shareLink:', `https://compiler-backend-e3eg.onrender.com/api/code/${token}`);
...
// // ... (keep Main App Component and export unchanged)
// // Main App Component
// const App = () => {
...
//   // Parse token from query params (for isShared case)
//   const searchParams = new URLSearchParams(location.search);
//   const token = searchParams.get('token');

//   // Use preloaded code from state if available, otherwise use template
//   const preloadedCode = location.state?.code;
//   const [code, setCode] = useState(preloadedCode || templates[language] || '');
//   const [output, setOutput] = useState('');
//   const [isError, setIsError] = useState(false);
//   const [shareLink, setShareLink] = useState('');
//   const [linkId, setLinkId] = useState('');
...
//   useEffect(() => {
//     if (isShared && token && !preloadedCode) {
//       console.log('EditorScreen: Fetching shared code for token=', token);
//       setIsLoading(true);
//       axios
//         .get(`https://compiler-backend-e3eg.onrender.com/api/code/${token}`, {
...
//         });
//     }
//   }, [isShared, token, language, preloadedCode]);
...
// // SharedCodeLoader Component
// const SharedCodeLoader = ({ mode, setMode }) => {
//   const location = useLocation();
//   const navigate = useNavigate();
...
//   useEffect(() => {
//     if (code && language) {
//       navigate(`/editor/${language}`, { state: { code } });
//     }
...
//                 element={
//                   <ErrorBoundary>
//                     <SharedCodeLoader mode={mode} setMode={setMode} />
//                   </ErrorBoundary>
//                 }
//               />
//               <Route path="*" element={<div><h2>404: Page Not Found</h2><button onClick={() => window.location.href = '/'}>Go Home</button></div>} />
